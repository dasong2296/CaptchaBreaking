...
raw_images = sorted(list(map(str, list(data_dir.glob("*.png")))))
labels = pd.DataFrame(data, columns=['Ground_Truth'])
characters = "0123456789ABCDEFGHJKLMNOPRSTUVWXZabcdefghijklmnopqrstuvwxyz"
Path("./CAPTCHAS_DATASET/pre_processed").mkdir(parents=True, exist_ok=True)
# pre-precessed
for i in raw_images:
    im = Image.open(i)
    im = prepare_image(im)
    im = remove_noise(im, 100)
    # im = im.filter(ImageFilter.MedianFilter(3))
    # im = im.filter(ImageFilter.GaussianBlur(3))
    # im = im.filter(ImageFilter.MedianFilter(5))
    im.save("./CAPTCHAS_DATASET/pre_processed/" + i.split('/')[-1])

print("Number of images found: ", len(raw_images))
